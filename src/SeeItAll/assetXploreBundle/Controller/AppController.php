...
public function urlLevel4Action($level0_id,$level1_id,$level2_id,$level3_id,$level4_id, Request $request)
{  
   $level4 = new Level4();
   $em = $this->getDoctrine()->getManager(); 
   ...
   $form_level4Url->handleRequest($request);   
   if ($form_level4Url->isSubmitted() && $form_level4Url->isValid()) {
   $em->persist($level4);
   $em->flush();
   // url redirection
